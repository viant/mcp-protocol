...
// UnmarshalJSON implements json.Unmarshaler.
func (j *ClientCapabilitiesUserInteraction) UnmarshalJSON(value []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(value, &raw); err != nil {
		return err
	}
	if _, ok := raw["types"]; raw != nil && !ok {
		return fmt.Errorf("field types in ClientCapabilitiesUserInteraction: required")
	}
	type Plain ClientCapabilitiesUserInteraction
	var plain Plain
	if err := json.Unmarshal(value, &plain); err != nil {
		return err
	}
	*j = ClientCapabilitiesUserInteraction(plain)
	return nil
}
